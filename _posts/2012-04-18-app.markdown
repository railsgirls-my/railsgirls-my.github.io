...
**Make sure you have a Nitrous.IO account.** Go to <a href="http://nitrous.io" target="_blank">http://nitrous.io</a> to sign up for one.
...
Next, type these commands in the *Console* tab of your Nitrous.IO page:
...
Click *Preview* at the top of the page and select *Port 3000*. You should see "Welcome aboard" page, which means that the generation of your new app worked correctly.
...
Click *Preview* at the top of the page and select *Port 3000* and append */ideas* to the
...
In your preview page, add new idea with a picture. When you upload a picture it doesn't look nice because it only shows a path to the file, so let's fix that.
...
If you try to click *Preview* at the top of the page and select *Port 3000* it still shows the "Welcome aboard" page. Let's make it redirect to the ideas page.
...
Test the change by refresh the preview page in your browser.
...
Now you can open the file `app/views/pages/info.html.erb` and add information about you in HTML and then click *Preview* at the top of the page and select *Port 3000* and append */pages/info* to the URL to see your new info page.
