...
Notice in the *Console* tab the command prompt is not visible because you are now in the Rails server. The command prompt looks like this:
{% highlight sh %}
$
{% endhighlight %}
...
Hit `CTRL-C` in the *Console* tab of the Nitrous.IO Web IDE to quit the server.
